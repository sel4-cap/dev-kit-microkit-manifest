...
1. Ensure that seL4 permits access to the necessary devices from the platform's device tree; this is performed through the configuration of the application's system file. For a worked example, see the  `uboot-driver-example.system` file from the `uboot-driver-example` [test application](uboot_driver_usage.md#test-application-uboot-driver-example).
